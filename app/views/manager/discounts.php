...
<div class="container">
    ...
    <h2>Total Price-wise Discounts</h2>
    <form class="discount-form" action="<?php echo URLROOT; ?>/managers/addtotaldiscount" method="POST" >
        <label for="total-discount">Enter Discount (%):</label>
        <?php if (!empty($data['discount_err'])) : ?>
            <span class="error"><?php echo $data['discount_err']; ?></span>
        <?php endif; ?>
        <input type="number" name="total_discount" id="total-discount" min="0" step="1" placeholder="Enter discount percentage"required>
        <?php if (!empty($data['menu_start_date_err'])) : ?>
            <span class="error"><?php echo $data['menu_start_date_err']; ?></span>
        <?php endif; ?>
        <label for="start-date">Start Date:</label>
        <input type="date" name="menu_start_date" id="start-date" min="<?php echo date('Y-m-d'); ?>"required>
        <?php if (!empty($data['menu_end_date_err'])) : ?>
            <span class="error"><?php echo $data['menu_end_date_err']; ?></span>
        <?php endif; ?>
        <label for="end-date">End Date:</label>
        <input type="date" name="menu_end_date" id="end-date" min="<?php echo date('Y-m-d'); ?>"required>
        <button type="submit">Apply Discount</button>
    </form>
